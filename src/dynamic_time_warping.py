...
def songRankingBaseline(hum, midi):
    iter = []

    for i in range (12):
        costs = []
        for j in range (len(midi)):
            costs.append(dynamicTimeWarping(hum, midi[j]))
        iter.append(costs)
        hum = transposeHum(hum)
...
def dynamicTimeWarping(arr1, arr2):
    cost_matrix = libfmp.c3.compute_cost_matrix(arr1, arr2)
    acc_cost_matrix = libfmp.c3.compute_accumulated_cost_matrix(cost_matrix)
    opt_path = libfmp.c3.compute_optimal_warping_path(acc_cost_matrix)

    return acc_cost_matrix[-1,-1]
